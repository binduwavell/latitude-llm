...
5.  **Start Services with Docker Compose**:
    This command starts the database, message queue, and other background services defined in `docker-compose.local.yml`.

    ```bash
    docker compose -f docker-compose.local.yml --profile local up -d # Run in detached mode
    ```
